...
public class LeaseAgreementController {
    ...
    public Result<List<AgreementItemVo>> listItem() {
        List<AgreementItemVo> list = leaseAgreementService.listItemByPhone(LoginUserHolder.getLoginUser().getUsername());
    ...
    public Result updateStatusById(@RequestParam Long id, @RequestParam LeaseStatus leaseStatus) {
        LambdaUpdateWrapper<LeaseAgreement> queryWrapper = new LambdaUpdateWrapper<>();
        queryWrapper.eq(LeaseAgreement::getId,id);
        queryWrapper.set(LeaseAgreement::getStatus,leaseStatus);
        leaseAgreementService.update(queryWrapper);
        return Result.ok();
    }
